...
ALTER TABLE platform ADD constraint platform_pk PRIMARY KEY CLUSTERED (id)
     WITH (
     ALLOW_PAGE_LOCKS = ON ,
     ALLOW_ROW_LOCKS = ON ) go
...
CREATE unique nonclustered index ticket__idxv3 ON ticket ( arrival_station_id ) go
...
ALTER TABLE track ADD constraint track_pk PRIMARY KEY CLUSTERED (id)
     WITH (
     ALLOW_PAGE_LOCKS = ON ,
     ALLOW_ROW_LOCKS = ON ) go
...
ALTER TABLE ticket
    ADD CONSTRAINT ticket_station_fk FOREIGN KEY ( departure_station_id )
        REFERENCES station ( id )
ON DELETE NO ACTION
    ON UPDATE no action go

ALTER TABLE ticket
    ADD CONSTRAINT ticket_station_fkv2 FOREIGN KEY ( arrival_station_id )
        REFERENCES station ( id )
ON DELETE NO ACTION
    ON UPDATE no action go
